...
//public 
	...
	public char searchPatternSplitter = ',';
	//Output
...
//Protected	
	...
	public void getFileList(DirectoryInfo di){
		...
		//get files
		List<FileInfo> fia = new List<FileInfo>();
		//Search all files divide by separator character
		foreach (string fileFormat in searchPattern.Split(searchPatternSplitter))
		{
			fia.AddRange(di.GetFiles(fileFormat));
		}
		//FileInfo[] fia = searchDirectory(di,searchPattern);
		files = new FileInformation[fia.Count];
		for(int f=0;f<fia.Count;f++){
			if(fileTexture)
